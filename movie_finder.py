...
for service in paid_services:
	...
	result[service_codes[service]] = service_list

free_services = ['hmf', 'pct', 'pcp', 'fmn', 'rkc', 'yfr', 'tcw', 'cws', 'vuf', 'pbs', 'tbv', 'knp', 'com', 'cru', 'abc', 'crk', 'his', 'aae', 'lft', 'pcf', 'bmg', 'dvc', 'ytv', 'mot', 'lol', 'ssc', 'ptv', 'vix', 'sft', 'tfd', 'vtv', 'ctw', 'ads', 'usn', 'vik', 'dkm', 'brv', 'fnw', 'ind', 'ahc', 'tlc', 'hgt', 'diy', 'inv', 'sci', 'dea', 'apl', 'dil', 'dis', 'coo', 'tra', 'hrv', 'ltv', 'vho', 'flr', 'rvy', 'rst', 'hyh', 'vrv', 'dpu', 'plx', 'own', 'fmz', 'bph', 'hoc', 'aim', 'pdm']
free_movie_list = []
indexes_to_remove = []
for i in range(len(watchlist_titles)):
	movie = watchlist_titles[i]
	movies_found = just_watch.search_for_item(query=movie, providers=free_services, monetization_types=['free', 'ads'])
	if len(movies_found['items'])>0 and movies_found['items'][0]['title'] == movie:
		offers = movies_found['items'][0]['offers']
		for offer in offers:
			if offer.get('monetization_type')=='ads' or offer.get('monetization_type')=='free':
				free_movie_list.append(service_codes[offer.get('package_short_name')]+': '+movie+' ('+str(movies_found['items'][0]['original_release_year'])+')') 
				indexes_to_remove.append(i)
				break
for i in sorted(indexes_to_remove, reverse=True):
	watchlist_titles.pop(i) # remove movie from overall watchlist
result['Free Services'] = free_movie_list
